...
In.ready('multipleDataBox','fileUtil','AllSelect','uploadBox','autocomplete','jqGrid','WdatePicker','SetJqGridList',function() {
	...
	$('.shouListBtn').toggle(
            function(){
                $(this).parents('.searchBox').find('.left ul .block').hide();
                $(this).find('.img').removeClass('fold');
                $(this).find('.img').addClass('unfold');
                $(this).find('.text').text('展 开');
            },function() {
                $(this).parents('.searchBox').find('.left ul .block').show();
		...
		if(id!=null && id!=''){
			...
			for(var i=0;i<dataJson.length;i++){
				...
				for( var aName in dataJson[i]){
					...
					var index = id.indexOf(dataJson[i].id);
					if(index >-1 ){	
...
*/						  default:return "";
			...
			/*loadComplete : function(data) {
				 $(".detail-btn").click(function(){
					var id =  $(this).attr("id");
					var purId = $("#pur_all").val();
				    window.location.href=appPath+"/backto/order/page/toOrderDetailsProcessing?id="+id+"&purchaserId="+purId;
				});
			 }*/
		});
...
//						setTimeout(WG.loading.hide(),5*1000);
		...
		function doExport(queryData,orderItemsIds,search,name,url){
	 	 ...
	 	 * 2016-9-27 yangtao
	 	 */
	 	$("#exportOrderSingle").click(function(){
...
//			console.log(queryData);
	 	 ...
	 	 */
	 	$("#exportOrderMultiple").click(function(){
	 		var orderItemsIds = getSelectIds("table");
			if(orderItemsIds==""){
				return ;
			}
...
/**
	...
	function saveSearch(){
		...
		urgentFlagId = $("input[name='urgentFlag']:checked").attr("id"),
