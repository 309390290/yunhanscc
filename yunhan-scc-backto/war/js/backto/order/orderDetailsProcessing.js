...
In.ready('multipleDataBox','autocomplete','uploadBox','bmz','jqGrid','WdatePicker','fileUtil','select','bmz',function() {
	//表格记录有问题的id数组
        ...
        colModel :[
            ...
            {name:"price", index:"定 价",width:'8%',align:'center',title:false,sortable:false,
            	formatter:function(value,options,rData){
            		...
            		if(rData.availablePrice!=null && rData.availablePrice!=""){
            			...
            			var div = "<div class='JGtab'><input type='text' value='"+availablePrice+"' class='b80' id='price_"+rData.id+"'></div>";
            			...
            			if(rData.isDeal=='Y'){
            				...
            				return "<div class='JGtab'><input type='text' value='"+price+"' class='b80' id='price_"+rData.id+"'></div>";
            			}
            	...
            	formatter:function(value,options,rData){
            		...
            		if(rData.historyAvailableDiscountrate !=null && rData.historyAvailableDiscountrate !=""){
            			...
            			if(value != rData.historyAvailableDiscountrate){
            				...
            				var div = "<div class='JGtab'><input type='text' value='"+availableDiscountrate+"' class='b80' id='discountrate_"+rData.id+"'></div>";
            				if(rData.isDeal=='Y'){
            				...
            				if(rData.isDeal=='Y'){
            					...
            					//存在暂存
                				if(rData.responseStatus==0){
                					return "<div class='JGtab'><input type='text' value='"+Number(rData.availableDiscountrate).toFixed(2)+"' class='b80' id='discountrate_"+rData.id+"'></div>";
                				}else{
                					return "<div class='JGtab'><input type='text' value='"+discount+"' class='b80' id='discountrate_"+rData.id+"'></div>";
                				}
            ...
            {name:"sendQtyTemp", index:"发货数暂存",width:'4%',align:'center',hidden:true,
            	formatter:function(value,options,rData){
            		if(rData.isDeal=='N'){
            			...
            			return "<input type='text' value='"+sendQtyTemp+"' class='b80' id='sendQtyTemp_"+rData.id+"'>";
            		}else{
            			return "";
            		}
            	}
            ...
            {name:"reasonValue", index:"其余满足情况",width:'8%',align:'center',hidden:true,
            	formatter:function(value,options,rData){
            		...
            		var html = "<input type='text' value='"+responseStatus+"' class='b80' id='responseStatus_"+rData.id+"'>" +
            				"<input type='text' value='"+reasonValue+"' class='b80' id='reasonValue_"+rData.id+"'>"
            		return html;
            	}
            },
...
//    					html +="<div class='JGtab'><input type='text' value='"+value+"' class='b80' id='preSendDate_"+rData.id+"'></div>";
    					html +="<span class='SetTiemBox'  id='preSendDate_"+rData.id+"'>"+value+"</span>";
    				}
    				return html;
            	}
            },
            ...
            {name:"isDealStr", index:"品种处理情况",width:'8%',align:'center',hidden:true,
            	formatter:function(value,options,rData){
            		return "<input type='text' value='"+rData.isDeal+"' class='b80' id='isDealStr_"+rData.id+"'>";
            	}
            },
...
//            $("#table").find(".cbox").trigger("click").prop('checked', true);
            ...
            //列表品种全部处理，将按钮置灰
            /*$("#sendGoods").attr("disabled",isDealFlag);
        	$("#save").attr("disabled",isDealFlag);
        	$("#OrderExport").attr("disabled",isDealFlag);
        	$("#OrderImport").attr("disabled",isDealFlag);
        	$("#batchInput").attr("disabled",isDealFlag);
        	$("#closeSend").attr("disabled",isDealFlag);*/
        },
    ...
    function sendoutGoods(ids){
		...
		for(var i=0; i< ids.length; i++){
			...
			if(otherAvailableReason!=null && otherAvailableReason!=''
				...
				msg ="回告永久缺货（改版、已停产、版权到期、商品无效、销售受限）的商品将自动关闭，除本次发货数外，其余未发数将无法再发货!仍然要继续吗?"
			}
			//处理数据
...
//    });
  ...
  //批量填写
    $("#batchInputSave").bind("click",function(){
    	/*var $this =  $("#batchInputForm");
    	var ids = $this.find("#batchIds").val().trim(),*///需要批量的记录id
    	var $this =  $("#batchInputForm");
//    	var ids = $("#table").jqGrid("getGridParam","selarrrow");//需要批量的记录id
    	//modify by lumin@20160901 解决ie8下不支持js的trim原生方法的兼容性问题。
    	//改为使用jquery的$.trim()方法
    	batchDiscount = $.trim($this.find("#batchDiscount").val()),
    	batchSendQty = $.trim($this.find("#batchSendQty").val()),
//    	batchDiscount = $this.find("#batchDiscount").val().trim(),//折扣
//    	batchSendQty = $this.find("#batchSendQty").val().trim(),//本次发货数
...
//    	batchRemark = $this.find("#batchRemark").val().trim();//备注
    	//排空处理
    ...
    $("#closeSend").bind("click",function(){
    	...
    	if(ids.length==0){
    		...
    		/*for(var i=0; i<ids.length; i++){
    			...
    			if(isDeal=='Y'){
    				layer.alert("已处理的商品不能进行【关闭发货】操作!", {icon : 0});
    				return ;
    			}
    		}*/
