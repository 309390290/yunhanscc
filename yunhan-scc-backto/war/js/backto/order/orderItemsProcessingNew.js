...
In.ready('multipleDataBox','AllSelect','autocomplete','select','bmz','uploadBox','jqGrid','WdatePicker','fileUtil','SetJqGridList',function() {
	//品种处理情况
	$('.selectDIY').find('span').click(function(){
        $(this).parents('.selectDIY').find('.oCon').show();
        return false;
    });
    $('.selectDIY').find('.oCon a').click(function () {
        $(this).parents('.oCon').hide();
    });
    $("html,body").click(function(){
        $('.selectDIY').find('.oCon').hide();
    });
    //当前页
	var tab = sessionStorage.getItem('dataTab');
	if($.trim(tab)!=""){
		$('#headActive li').removeClass('active');
		$('.bodyContentBox .Tab').removeClass('active');
		$('.bodyContentBox .Tab').eq(tab).addClass('active');
		$('#headActive li').eq(tab).addClass('active');
		   var left=$('#headActive li').eq(tab).offset().left,
	        width=$('#headActive li').eq(tab).innerWidth();
	    $('#activeNow').css({'left':left,'width':width})
	}
	var bmz_yz=[];
    ...
    tabCom= $('.bodyContentBox .Tab');//tab内容
		//tab切换
		tabActive.click(function(){
		    ...
		    tabCom.eq($(this).index()).addClass('active');
		    tabCom.hide();
		    tabCom.eq($(this).index()).show();	
		    var left=$(this).offset().left,
		        width=$(this).innerWidth();
		    $('#activeNow').animate({'left':left,'width':width})
		    //每次点击标签页都要重新加载一次数据
			 var iNow=$(this).index();
				if (0 == iNow) {            // 待处理品种
					sessionStorage['dataTab']='0';
					$('#table').setGridWidth($('#table').parents('.jqGrid').eq(0).width());
				}
				if (1 == iNow) {            // 已处理品种查询
					sessionStorage['dataTab']='1';
					$('#table_a').setGridWidth($('#table_a').parents('.jqGrid').eq(0).width());
				}
		});
	/***********tab切换事件 end**********/
	//初始化采购商查询条件
...
//		sessionStorage.clear();
		...
		if(isDeal=="Y"){
			    ...
			    var left=tabActive.eq(1).offset().left,
		        	width=tabActive.eq(1).innerWidth();
        ...
        colModel :[
            ...
            {name:"purchaseOrderCode", index:"订单号",width:'10%',align:'center',sortable: false,
            	formatter:function(value,options,rData){
				 ...
				 // alert(order_type);
				  ...
				  //已下载
	  			 if(rData.isExport == 'Y'){
	  				 htmls += "<div class='iconyxz'><i class='u-icon-yxz'>已下载</i></div>";
	  			 }
	  			  //订单号和订单类型
	  				htmls +="<div><a class=\"orderDetail-btn\" herf='#' id=\""+rData.summaryId+"\">"+value+"</a><input type='hidden' value='"+value+"' id='purchaseOrderCode_"+rData.id+"'></div>" +
	  						"<div class='uicon'><i class='u-icon-orderType'>"+order_name+"</i>";
				  //紧急  
            ...
            {name:"isbn", index:"商品信息",width:'16%',align:'center',sortable: false,
            	formatter:function(value,options,rData){
            		...
            		html +="</ul></div>";
            		//书名部分
            		html+="<div class='right'><ul class='newline'><li>"+rData.bookTitle+"</li>"
            		...
            		formatter:function(value,options,rData){
            			...
            			if(rData.historyAvailablePrice!=null && rData.historyAvailablePrice!=""){//已回告
            				...
            				if(rData.availablePrice!=null && rData.availablePrice!=""){
            					html+="<div class='JGtab'><input type='text' id='price_"+rData.id+"' value='"+Number(rData.availablePrice).toFixed(2)+"' class='b80 inputWidth' last-price='"+Number(rData.availablePrice).toFixed(2)+"' ></div>";
            				}else{
            					html+="<div class='JGtab'><input type='text' id='price_"+rData.id+"' value='"+Number(price).toFixed(2)+"' class='b80 inputWidth' last-price='"+Number(price).toFixed(2)+"' ></div>";
            				}
            			}
                	    return html;
                	}},
                			...
                			if(rData.historyAvailableDiscountrate!=null && rData.historyAvailableDiscountrate!=""){//已回告
                				if(value!=rData.historyAvailableDiscountrate){
                					if(rData.availableDiscountrate!=null && rData.availableDiscountrate!=""){//暂存不为空，显示暂存折扣，否则显示历史回告折扣
                						...
                						html+="<div class='JGtab'><input type='text'    id='discountrate_"+rData.id+"'    value='"+Number(rData.availableDiscountrate).toFixed(2)+"' class='b80 inputWidth' last-discountrate='"+Number(rData.availableDiscountrate).toFixed(2)+"'></div>"
                						html+="</div>";
                					}else{
                						html+="<div class='fontdel'>订单折扣："+Number(value).toFixed(2)+"<div>";
                						html+="<div class='JGtab'><input type='text'    id='discountrate_"+rData.id+"'    value='"+Number(rData.historyAvailableDiscountrate).toFixed(2)+"' class='b80 inputWidth' last-discountrate='"+Number(rData.historyAvailableDiscountrate).toFixed(2)+"'></div>"
                						html+="</div>";
                					}
                    			}else{
                    				if(rData.availableDiscountrate!=null && rData.availableDiscountrate!=""){//暂存不为空，显示暂存折扣，否则显示订单折扣
                    					html+="<input type='text'  id='discountrate_"+rData.id+"'   value='"+Number(rData.availableDiscountrate).toFixed(2)+"' class='b80 inputWidth' last-discountrate='"+Number(rData.availableDiscountrate).toFixed(2)+"'></div>";
                    				}else{
                    					html+="<input type='text'  id='discountrate_"+rData.id+"'   value='"+Number(rData.historyAvailableDiscountrate).toFixed(2)+"' class='b80 inputWidth' last-discountrate='"+Number(rData.historyAvailableDiscountrate).toFixed(2)+"'></div>";
                    				}
                    			}
                			}else{
                				if(rData.availableDiscountrate!=null && rData.availableDiscountrate!=""){
                    				...
                    				html+="<input type='text' id='discountrate_"+rData.id+"'  value='"+Number(value).toFixed(2)+"' class='b80 inputWidth' last-discountrate='"+Number(value).toFixed(2)+"'></div>";
                				}
                			}
                    	    return html;
                    	}},
            ...
            {name:"thisSendQty", index:"本次发货数",width:'6%',align:'center',sortable: false,
        	 ...
        	 {name:"reasonValue", index:"其余满足情况",width:'8%',align:'center',hidden:true,
            	formatter:function(value,options,rData){
            		...
            		var html ="<input type='text' value='"+reasonValue+"' class='b80 inputWidth' id='reasonValue_"+rData.id+"'>";
            		return html;
            	}
            },
            ...
            {name:"preSendDate", index:"其余预计发货日期",width:'8%',align:'center',sortable: false,
                    	formatter:function(value,options,rData){
                    		...
                    		if(rData.isDeal=='N'){
                    			...
                    			html +="<div class='JGtab'><input type='text' value='"+value+"' class='timeIcon b90 inputWidth' id='preSendDate_"+rData.id+"'></div>";
                    			return html;
                    		}else{
                    			return "";
                    		}
            ...
            {name:"status", index:"回告历史",width:'5%',align:'center',sortable: false,
            	formatter:function(value,options,rData){
            		if(value ==null){
            			...
            			return "<a item-id='"+rData.id+"' class=\"showBackToHistory\">查 看</a>";
            		}
            	}
            }
        ],// the column discription
...
//								search();
						...
						WG.loading.hide();
						layer.msg("长时间未操作，平台已断开，请重新登录!",{icon : 0,time:2500});
					}
	  			});
		...
		var flag = false;
 		...
 		$.ajax({
			...
			success : function(data) {
				...
				var wareHouseReArr = wareHouseRe.split(',');
				if(data){
					for(var i=0;i<data.length;i++){
						var isEqualHouse = false;
						if(wareHouseRe != ""){
							for(var j in wareHouseReArr){
								if(data[i].code.indexOf(wareHouseReArr[j])>=0){
						...
						if((wareHouseRe!=null || wareHouseRe!="")&& isEqualHouse){
...
function saveSearch(){
	...
	urgentFlagTwo =$("#urgentFlagTwo").val();
	
	var isNewId = $("#isNew").val(),
	isExport = $("#isExport").val(),
	urgentFlag = $("#urgentFlag").val(),
//	controlFlag = $("input[name='controlFlag']:checked").attr("id"),
	
	isExportTwo = $("#isExportTwo").val(),
	isNewTwo = $("#isNewTwo").val();
