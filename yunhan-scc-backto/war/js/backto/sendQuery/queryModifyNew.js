...
In.ready('jqGrid','multipleDataBox','select','fileUtil',function() {
	...
	var purchaserId=$("select[id=purchaserId]").val();
	getDC(purchaserId);
	/**
	 * 采购商
	 */
   ...
   $('.del').click(function(){
	...
	// 采购商改变事件，重新加载仓位和表单
	$("#purchaserId").change(function(){
		var purId=$("select[id=purchaserId]").val();
		getDC(purId);
 	});
	...
	/**
 	...
 	$("a[name='sendDate']").click(function(){
		...
		if('monthOneAgo' == thisId){
			startDt = '';
			endDt = GetDateStr(-30);
		}else if('monthOne' == thisId){
			...
			success : function(data) {
				...
				if(data){
					for(var i=0;i<data.length;i++){
						var sel = "";
						options += "<option value='"+data[i].code+"' "+sel+" paddr='"+data[i].paddr+"' contact='"+data[i].contact+"' >"+data[i].name+"</option>";
					}
					$("#dc_all").html(options);
				}
			},error : function() {
	...
	$("#table_list").jqGrid({ 
		...
		datatype: "json", //数据格式
		mtype:"POST", 
		colNames:["ID","供应商ID","采购商ID","主发标识","发货单号Back","发货单号","制单日期","采购商","仓位","合计发货品种","总册数","总码洋","总实洋","发货日期","发货单状态","操 作"],// the column header names  
	     colModel :[ 
			  ...
			  {name:"sendoutGoodsCode", index:"sendoutGoodsCode",width:'15%',
	        	  formatter : function(value, options, rData) {
	        		  if(rData.isInitiative=="Y" ){
						  return "<i class=\"u-icon-zf\">主发</i><a class='line sgc-btn' id=\""+options.rowId+"\" href='#'>"+value+"</a>";
					  }else if(rData.isSupplierAddProduct=="Y"){
	        			  return  "<i class='u-icon-zf'>添加</i> <a class='line sgc-btn' id=\""+options.rowId+"\" href='#'>"+value+"</a>";
	        		  } 
					  return  "<a class='line sgc-btn' id=\""+options.rowId+"\" href='javaScript:void(0);'>"+value+"</a>";
	        	  }, align:'center'}, 
	          {name:"addTime", index:"addTime",width:'8%',align:'center', sorttype:'date', formatter:'date', formatoptions:{srcformat:'Y-m-d H:i:s', newformat:'Y-m-d'}},   
			  {name:"purchaserName", index:"purchaserName",width:'18%',align:'center'}, 
			  {name:"receiveWarehouseName", index:"receiveWarehouseName",width:'5%',align:'center'}, 
			  ...
			  {name:"totalPrice", index:"totalPrice",width:'9%',align:'center', sorttype:'number'}, 
			  {name:"totalRealityPrice", index:"totalRealityPrice",width:'9%',align:'center', sorttype:'number'}, 
			  {name:"sendoutDate", index:"sendoutDate",width:'8%',align:'center', sorttype:'date', formatter:'date', formatoptions:{srcformat:'Y-m-d H:i:s', newformat:'Y-m-d'}},   
			  {name:"sendoutStatus", index:"sendoutStatus",width:'8%',
				  formatter : function(value, options, rData) {
					  if(value==0)
						  return "未发货";
					  if(value==5)
			  ...
			  {name:"name15", index:"操 作",width:'7%',
				  formatter : function(value, options, rData) {
					  if(rData.sendoutStatus==0 || rData.sendoutStatus==5){
						  return "<span id='"+rData.id+"' class='modify-btn closeBtn'>修改</span>";
					  }else{
						  return "<span disabled='true' class='closeBtnT modify-btn'>修改</span>";
					  }
				  },align:'center'}
			],
	   	rowNum: 50,  //默认分页行数
  	...
  	function queryData(){
  	   ...
  	   //制单日期to
  		...
  		//收货仓位
  		var receiveWarehouse    = $("select[id=dc_all]").val();
  		if(null != receiveWarehouse && receiveWarehouse.length>0 ){
  			var addDateToQC = "{'field':'receiveWarehouses','op':'eq','data':'"+ receiveWarehouse +"'},";
  			queryData = queryData.concat(addDateToQC);
  		}
  		//主发
